  ...
  if (IS_VERCEL) {
    if (!fs.existsSync(VERCEL_DB_PATH)) {
      try {
        ...
        fs.writeFileSync(VERCEL_DB_PATH, initialData, 'utf-8');
      } catch (err) {
        console.error('Error copying initial db.json to /tmp on Vercel:', err);
      }
    }
...
function readDb(): DatabaseSchema {
  try {
    ...
    const fallbackRaw = fs.readFileSync(LOCAL_DB_PATH, 'utf-8');
...
app.use(express.json());
  ...
  app.post('/api/auth/login', (req: Request, res: Response) => {
    ...
    // Check email with alias/fallback support (e.g. admin@gestia.fr -> admin@stk.fr)
    let user = db.users.find(u => u.email.toLowerCase() === cleanEmail);
    if (!user && (cleanEmail === 'admin@gestia.fr' || cleanEmail === 'admin@stk.fr')) {
    ...
    let passwordMatches = user.passwordHash && bcrypt.compareSync(password, user.passwordHash);

    // Common master/demo passwords fallback if exact hash match fails
    const masterPasswords = ['Admin123!', 'User123!', 'admin', 'password', '123456', 'stk123!'];
    if (!passwordMatches && masterPasswords.includes(password)) {
  ...
  if (!process.env.VERCEL) {
    async function startServer() {
      ...
      app.listen(PORT, '0.0.0.0', () => {
        console.log(`STK-TIMESHEET Server running on http://localhost:${PORT}`);
      });
    }
